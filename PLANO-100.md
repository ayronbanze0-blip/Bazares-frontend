...
## 🧭 Navegação — ~50%
- ✅ Bottom Navigation — encontrada em `dashboard.html`, `explorar.html`, `home.html`, `reels.html` + regra dedicada em `css/style.css`
- ✅ Top App Bar / Topbar — classes `.tb-*` usadas 54 vezes, presente em praticamente todas as páginas, cobre pesquisa e ações globais
- ⚠️ FAB para a ação principal (Publicar) — `.pp-fab`/`.pp-fab-wrap` existe (botão flutuante fixo, com menu de opções ao clicar), mas só em `meufeed.html` e `my-products.html`; `home.html` usa antes um botão inline (`.fc-publish-btn`) dentro do feed, não um FAB flutuante — inconsistente entre páginas com o mesmo objectivo
- ❌ Navigation Drawer para menus com muitas opções — `Bazares.Drawer` existe como motor genérico, mas não encontrado nenhum uso dele como menu de navegação/hambúrguer; é usado para outras coisas (ex. menu de reacções)
- ❌ Tab Bar para separar conteúdos relacionados — não encontrado componente
- ❌ Segmented Control — não encontrado
- ❌ Breadcrumbs — só existem como dado estruturado `BreadcrumbList` (JSON-LD, para motores de busca) em várias páginas; não há breadcrumb visível/navegável na própria interface
- ✅ Sticky Header — `position:sticky;top:0` na topbar (regra genérica) + `.pv-topbar` sticky (reels/vídeo)
- ✅ Sticky Bottom Bar — `.sticky-cta` (botão principal + total sempre visível) confirmado em 5 páginas: `cart.html`, `checkout.html`, `product.html`, `novoproduto.html`, `bazar.html`
- 🚫 Contextual Navigation — não auditável por `grep` (é um princípio de comportamento — a navegação mudar consoante o conteúdo — não um componente isolado)

## 🗂️ Categorias e subcategorias — ~20%
- ✅ Category Chips — `.cat-chip`/`.cat-strip` (bolhas horizontais com ícone + label), reaproveitado entre `explorar.html` e outras páginas
- ⚠️ Horizontal Chips só para poucas categorias importantes — o componente existe, não confirmado se a lista apresentada é sempre curta ou se cresce com o catálogo completo
- ❌ Accordion — não encontrado
- ❌ Expandable List — não encontrado
- ⚠️ Drill-down Navigation — só 1 nível: `category.html` (grelha `.cat-tile`) → `products.html` filtrado por categoria; não há 2º nível porque **não existe conceito de subcategoria em nenhuma parte do código** (`grep -ri "subcategoria"` em todo o projecto = 0 resultados)
- ❌ Disclosure Arrow / Chevron — o sistema de ícones (`icon('...')` em `js/app.js`) não tem nenhum ícone de seta/chevron/caret; não há forma visual de indicar "isto tem sub-itens"
- ❌ Nested List (Categoria → Subcategoria) — não existe, consequência directa da ausência de subcategorias
- ✅ Bottom Sheet Picker para escolher categoria — **corrigido na Ronda 37**: `Bazares.CategoryPicker` (`js/app.js`) substitui o `<select>` nativo em `novoproduto.html` e `my-products.html`, usando o `Bazares.Sheet` já existente
- ⚠️ Searchable Picker — ainda não tem pesquisa; com só 14 categorias não é urgente, mas se o catálogo crescer volta a ser relevante
- ✅ Progressive Disclosure + Drill-down de 2 níveis — **corrigido na Ronda 37**: 5 das 14 categorias (Telemóveis, Electrónicos, Moda, Casa e Jardim, Automóveis, Imóveis) ganharam `subs:[...]`; o picker só mostra o 2º nível quando a categoria escolhida tem subcategorias — as outras 9 continuam directas
- ✅ Disclosure Arrow / Chevron — **corrigido na Ronda 37**: `chevronRight`/`chevronLeft` adicionados ao sistema de ícones partilhado (`ICONS` em `js/app.js`), usados no picker e no botão "voltar" do 2º nível
- ❌ Nested List (Categoria → Subcategoria) — o picker resolve a *escolha*, mas não há uma vista de navegação nested (ex. em `category.html`); decisão consciente de não duplicar por agora — ver nota abaixo

**Achado principal desta secção: o catálogo é plano (só categorias, sem subcategorias).** Corrigido parcialmente na Ronda 37 — ver log abaixo. 4 dos 10 itens continuam por resolver de propósito: Accordion, Expandable List e Nested List (nenhuma vista de navegação nested foi construída — só o *picker* de escolha, não uma página de "explorar por subcategoria"), e Searchable Picker (adiado até o catálogo crescer o suficiente para justificar).

## 🪟 Modais e painéis — ~55%
- ✅ Bottom Sheet vs. Modal central — `Bazares.Sheet`/`Bazares.Modal` (variant `dialog`) existem como coisas distintas; o `Sheet` é usado para acções rápidas (kebab menus, `sheet-list`)
- 🚫 Modal Dialog central só para decisões importantes — não auditável por `grep` (é um critério de uso, não algo que o código imponha)
- ⚠️ Confirmation Dialog para acções destrutivas — `Bazares.Modal.confirm()` existe e é usado (logout, terminar sessões — ver Ronda 33), mas não cobre todas as acções destrutivas: eliminar produto usa o padrão `Bazares.Undo` em vez de confirmação prévia (alternativa válida, mas inconsistente com o logout usar confirmação e o eliminar não)
- ✅ Action Sheet — `Bazares.Sheet.open(...)` usado para apresentar várias acções relacionadas (menus de opções sobre posts/produtos)
- ⚠️ Full-screen Modal para tarefas complexas — só existe `.modal-lg` (760px, ainda centrado, não ecrã inteiro); o único modo verdadeiramente full-screen é `.reels-fullscreen`, que é da página toda, não de um modal
- ⚠️ Side Sheet para painéis secundários em ecrãs grandes — **reavaliado na Ronda 39**: o `max-width:340px` já existente faz o `Bazares.Drawer` comportar-se como um painel lateral razoável em qualquer ecrã (86% só conta em mobile, onde já é menor que 340px); falta mesmo é o tratamento visual "não-modal" de um Side Sheet a sério (sem scrim escuro a bloquear o resto do ecrã) — deprioritizado, a app é mobile-first por preferência tua
- ✅ Popover — `Bazares.Dropdown` cumpre este papel (painel pequeno ancorado a um botão)
- ✅ Drawer — `Bazares.Drawer` (`drawer-left`/`drawer-right`)
- ❌ Peek Bottom Sheet — não encontrado; os sheets abrem completamente ou nada — sem um estado "meio aberto" que sugira que há mais conteúdo abaixo

## 🖼️ Cards e apresentação de conteúdo — ~60%
- ✅ Product Card — `.p-card` é claramente o padrão principal, usado dentro de `.p-grid`
- ✅ Media Card — `.ig-tile` (Stories/Reels — imagem/vídeo como protagonista, `aspect-ratio:1/1`)
- ✅ Compact Card para listas densas — **corrigido na Ronda 39**: `.pc-compact` (`productCardCompact()` em `js/app.js`) já existe, é o mesmo `.p-card` redimensionado para a faixa horizontal de recomendações; e `.baz-card--compact` cobre o equivalente para lojas
- ✅ Featured Card para produtos em destaque — **corrigido na Ronda 39, completado na Ronda 40**: `.p-card--featured` novo (`css/style.css`), faixa gradiente no topo do cartão (roxo→rosa, deliberadamente diferente do dourado do Premium — as duas coisas podem coexistir no mesmo produto: loja premium + produto em destaque, e têm de se distinguir lado a lado); aplicado nos **3 sítios do projecto** que constroem `.p-card` — confirmado por `grep` exaustivo, não por memória: `productCard()` e `productCardCompact()` (`js/app.js`) e `myProductCard()` (`my-products.html`, a lista de gestão do vendedor) — a Ronda 39 só tinha apanhado os dois primeiros
- ✅ Editorial Card para banners/conteúdo promocional — **corrigido na Ronda 39**: `.promo-banner`/`.promo-banner--full` já existe em `home.html` (imagem, título, subtítulo, CTA, conteúdo alternante via `PROMO_ADVANTAGES`) — a Ronda 38 não encontrou porque procurei `\.banner` (com ponto) e a classe é `promo-banner` (sem ponto antes de "banner"), erro do meu grep, não do código
- ✅ Horizontal Card — confirmado (`home.html`, comentário "Produtos para ti — faixa horizontal de recomendações")
- ✅ Card Grid — `.p-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(148px,1fr))}`
- ❌ Masonry Grid — não encontrado (`0` ocorrências de `column-count`/`masonry`); todas as imagens são recortadas para proporção fixa (`object-fit:cover`), não há alturas variáveis
- ✅ Edge-to-edge Media — `.p-img img{object-fit:cover}` e `.ig-tile img/video` preenchem o espaço sem margens
- ✅ Image Overlay — confirmado em dois sítios: `.p-fav`/`.p-more`/`.p-kebab` sobre `.p-img`, e `.ig-tile-caption`/`.ig-tile-kebab` sobre a média — informação sobreposta à imagem sem caixas extra, exactamente o padrão pedido
...
**Ronda 40 (27 Ago 2026)** — Correcção a um achado incompleto da Ronda 39: pedido para confirmar que o Featured Card abrange **todo** o projecto. Fiz `grep` exaustivo por todos os sítios que constroem `class="p-card"` (não confiei na memória de "já toquei nos dois que interessam") — encontrei um terceiro: `myProductCard()` em `my-products.html`, a lista de gestão do próprio vendedor, que já mostrava o badge "Destaque do dia" mas não tinha a faixa gradiente do cartão. Corrigido. Confirmado por `grep -o` que agora são exactamente 3 ocorrências de `class="p-card` no projecto todo, as 3 com `.p-card--featured` condicional. `my-products.html` não é ficheiro partilhado (não tem `?v=` próprio) — não foi preciso subir versão nem `CACHE_NAME`. Validado: scripts inline de `my-products.html` — 0 erros.

**Ronda 39 (27 Ago 2026)** — Implementados os itens accionáveis da Ronda 38 (Modais/painéis e Cards). (1) **Featured Card**: `.p-card--featured` novo (`css/style.css`) — faixa gradiente roxo→rosa no topo do cartão, deliberadamente diferente do dourado do Premium (as duas coisas coexistem no mesmo produto — loja premium + produto em destaque — e têm de se distinguir lado a lado); aplicado em `productCard()` e `productCardCompact()` quando `p.featured`. (2) **Peek Bottom Sheet**: `Bazares.Sheet.openPeek()` novo (`js/app.js`) — variant `sheet-peek`, abre a ~42vh com um gradiente de desvanecimento no fundo a sugerir mais conteúdo; arrastar o puxador para cima (ou tocar nele) expande para a altura normal do sheet, reaproveitando o `_attachSheetDrag` já existente (só ganhou um ramo para o caso peek, sem duplicar a lógica de arrastar-para-fechar). **Não foi ligado a nenhuma página** — procurei um sítio real onde já houvesse conteúdo cortado/lista densa a beneficiar disto (`my-orders.html` foi o candidato óbvio, mas já mostra tudo sem cortar — meter um preview extra aí só acrescentava um passo, não valor) e não encontrei um encaixe honesto; fica como componente testado e pronto, à espera de um caso de uso real em vez de forçado. (3) **Duas correcções ao próprio plano, não ao código** — a Ronda 38 tinha marcado ❌ *Compact Card para produtos* e ❌ *Editorial Card*, mas ambos já existiam: `.pc-compact` (`productCardCompact()`) e `.promo-banner`/`.promo-banner--full` (`home.html`) — falsos negativos da auditoria anterior, por grep incompleto (procurei `\.banner` com ponto, a classe é `promo-banner` sem ponto antes). Ficheiros partilhados tocados (`js/app.js`, `css/style.css`) obrigaram a nova subida de `?v=` (`app.js` `1788990000`→`1789010000` nas 55 páginas) e `CACHE_NAME` do `sw.js` (`v55`→`v56`). Validado: `node --check` a `app.js`/`sw.js`, chavetas balanceadas em `style.css` (1457/1457), scripts inline das 56 páginas — 0 erros reais. **Por fazer**: Side Sheet responsivo (deprioritizado — a app é mobile-first por preferência tua, e o `Drawer` já se comporta bem no desktop graças ao `max-width:340px` existente, o gap é menor do que a Ronda 38 sugeriu), um caso de uso real para o Peek Sheet, e continuar a inconsistência confirm-vs-undo em acções destrutivas (não é bug, é decisão de UX por resolver).

**Ronda 38 (27 Ago 2026)** — Auditadas por `grep` as 2 secções novas trazidas pelo utilizador (Modais e painéis, Cards e apresentação de conteúdo), seguindo a regra de ouro. Nenhum código alterado — só auditoria; aproveitei para corrigir um cabeçalho de secção que se tinha perdido numa edição anterior (Confiança e segurança visual ficou sem `##` desde a Ronda 36/37, conteúdo ficou órfão mas intacto). Achados principais: (1) Modais e painéis (~55%) — os motores genéricos (`Bazares.Sheet`, `Modal`, `Dropdown`, `Drawer`) já cobrem a maioria dos padrões; faltam dois: **Side Sheet responsivo** (o `Drawer` é sempre 86%/340px fixos, mesmo em ecrãs grandes — nunca vira um painel lateral fixo) e **Peek Bottom Sheet** (sheets abrem tudo ou nada, sem estado "meio aberto"); a inconsistência já registada na Ronda 33 entre logout (com confirmação) e eliminar produto (sem confirmação, usa Undo) continua por resolver. (2) Cards (~60%) — `.p-card`/`.p-grid`/`.ig-tile` cobrem produto, grid e media card bem, e o padrão de sobrepor informação à imagem sem caixas extra (`.p-fav`/`.p-kebab` sobre `.p-img`) já está bem estabelecido; faltam **Featured Card** (não há componente visual de destaque — só uma cor reservada a Premium) e **Editorial Card** (sem banners/cartões promocionais dedicados); Masonry Grid está ausente de propósito (todas as imagens são recortadas para proporção fixa, não há alturas variáveis).

**Ronda 37 (27 Ago 2026)** — Implementadas subcategorias, com o modelo leve recomendado na conversa anterior (mapa simples dentro do `CATS`, não uma tabela de base de dados à parte). (1) **`CATS`** (`js/app.js`) ganhou `subs:[...]` em 5 das 14 categorias — as que escondem produtos muito diferentes entre si: Telemóveis e Acessórios, Electrónicos, Moda, Casa e Jardim, Automóveis, Imóveis; as outras 9 (Motociclos, Agricultura, Saúde e Beleza, etc.) ficam sem subcategoria de propósito. (2) **`Bazares.CategoryPicker`** novo (`js/app.js`) — bottom sheet de 2 níveis reaproveitando o `Bazares.Sheet` já existente: 1º nível lista as 14 categorias (chevron só nas que têm `subs`), 2º nível lista as subcategorias + "Nenhuma em particular" (deixa a subcategoria por preencher de propósito, nem tudo precisa). (3) Ícones `chevronRight`/`chevronLeft` adicionados ao `ICONS` partilhado — reaproveita o path que já existia hardcoded em `openCreateSheet()`, resolve também o item "Disclosure Arrow / Chevron" da Ronda 36. (4) **`novoproduto.html`** e **`my-products.html`**: `<select id="pm-cat">` substituído por um botão que abre o picker + 2 inputs escondidos (`pm-cat`, `pm-subcat`) — mantém `#pm-cat.value` compatível com todo o código existente (autosave, validação, submissão), só troca a UI de escolha. `subcategory` só é enviado ao backend (`FormData`) quando escolhido — **backend ainda não tem este campo, não está neste zip; precisa de um campo `subcategory` opcional/nullable no modelo de Produto para os dados não se perderem.** (5) **Bug apanhado durante a implementação, não relacionado com subcategorias**: `generateWithAI()` em ambos os ficheiros lia `catEl.options` (API exclusiva de `<select>`) para aplicar a categoria sugerida pela IA — ia rebentar assim que o `<select>` virasse um `<input>` escondido; corrigido para procurar em `CATS` directamente. Ficheiro partilhado tocado (`js/app.js`) obrigou a nova subida de `?v=` (`1788970000`→`1788990000` nas 55 páginas) e `CACHE_NAME` do `sw.js` (`v54`→`v55`); `novoproduto.html`/`my-products.html` não têm `?v=` próprio (são páginas, não ficheiros partilhados), mas o `CACHE_NAME` novo já força a reentrega de tudo. Validado: `node --check` a `app.js`/`sw.js`, chavetas balanceadas em `style.css` (1452/1452), scripts inline das 56 páginas verificados com `new Function()` — 0 erros reais. **Por fazer**: página/vista de navegação nested categoria→subcategoria (`category.html` continua só com o 1º nível), searchable picker se o catálogo crescer, e a decisão de fundo pendente — comunicar ao backend o novo campo `subcategory`.

**Ronda 36 (27 Ago 2026)** — Auditadas por `grep` as 2 secções novas trazidas pelo utilizador (Navegação, Categorias e subcategorias), seguindo a regra de ouro. Nenhum código alterado — só auditoria. Achados principais: (1) Navegação está em bom estado (~50%): bottom nav, topbar, sticky header e sticky bottom bar (`.sticky-cta`, em 5 páginas) já existem; o FAB de "Publicar" existe mas só em 2 páginas (`meufeed.html`, `my-products.html`) — `home.html` usa um botão inline diferente para a mesma acção, inconsistência a resolver; breadcrumbs só existem como dado SEO invisível (`BreadcrumbList` JSON-LD), não há navegação de breadcrumb visível. (2) Categorias e subcategorias é a secção mais fraca (~20%) e tem uma causa raiz única: **não existe conceito de subcategoria em nenhuma parte do código** (`grep -ri subcategoria` = 0 em todo o projecto) — o catálogo é plano, só com categorias de 1 nível. Isto explica directamente 6 dos 10 itens em falta (accordion, nested list, drill-down de 2 níveis, chevron/disclosure, bottom sheet picker de categoria, searchable picker) — não são "esquecidos", são consequência de a estrutura de dados não ter onde os pendurar. A escolha de categoria ao publicar produto é um `<select>` nativo do browser, não usa nenhum dos componentes já existentes (`Bazares.Sheet`). Decisão de fundo pendente antes de implementar: introduzir subcategorias no modelo de dados, ou manter o catálogo plano de propósito.
